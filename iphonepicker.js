...
(function (w, $, o) {
	...
	$.extend(o, {
		...
		init: function (target, options) {
			...
			if ($('#uipv_main_' + target[0].id).length === 0) {
		...
		addNiftyControl: function (data) {
			...
			var controlWidth = (data.options.width === '') ? o.stripSizePostFix($('#'+targetId).css('width')) : o.stripSizePostFix(data.options.width);
...
(function (w, m, $, o) {
	...
	$.extend(o, {
      ...
      if (data === null){
			...
			$('select#'+data.options.parentId+' option:selected').removeAttr('selected');
			$('select#'+data.options.parentId+' option').eq(index).attr('selected', 'selected');
